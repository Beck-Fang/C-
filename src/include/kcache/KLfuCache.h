...
template <typename Key, typename Value>
class KLfuCache : public CachePolicy<Key, Value>
{
...
public:
    ...
    explicit KLfuCache(int capacity, int maxAverageNum = 1000000, const EvictedFunc &evicted_func = nullptr)
    : capacity_(capacity), minFreq_(INT8_MAX), maxAverageNum_(maxAverageNum),
      curAverageNum_(0), curTotalNum_(0), evicted_func_(evicted_func)
    {}
    ...
    bool Delete(Key &key) override
    {
      ...
      {     
        ...
        if (this->evicted_func_ != nullptr)
            this->evicted_func_(node->key, node->value);
      }
      return true;
    }
...
private:
    ...
    std::unordered_map<int, std::shared_ptr<FreqList<Key, Value>>>  freqToFreqList_;// 访问频次到该频次链表的映射
...
template<typename Key, typename Value>
class KHashLfuCache : public CachePolicy<Key, Value>
{
    using EvictedFunc = std::function<void(Key, Value)>;
    using ValueOptional = std::optional<Value>;

public:
    KHashLfuCache() = delete;
    explicit KHashLfuCache(size_t capacity, int sliceNum = -1, int maxAverageNum = 1000000,const EvictedFunc &evicted_func = nullptr)
        ...
        , capacity_(capacity)
    {
        ...
        {
            lfuSliceCaches_.emplace_back(std::make_unique<KLfuCache<Key, Value>>(sliceSize, maxAverageNum,evicted_func));
        }
    }
    ...
    bool Set(Key &key, Value value)override
    {
        ...
        lfuSliceCaches_[sliceIndex]->Set(key, value);
        return true;
    }
    ...
    bool Delete(Key& key)override
    {
        // 根据key找出对应的lfu分片
        size_t sliceIndex = Hash(key) % sliceNum_;
        lfuSliceCaches_[sliceIndex]->Delete(key);
        return true;
    }
