...
public:
    ...
    explicit ArcLruPart(size_t capacity, size_t transformThreshold, const EvictedFunc& evicted_func = nullptr)
        : capacity_(capacity)
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        auto main_it = mainCache_.find(key);
        if(main_it != mainCache_.end())
        {
            removeFromMain(main_it->second);
            mainCache_.erase(main_it);
            if(this->evicted_func_ != nullptr)
                this->evicted_func_(key, main_it->second->getValue());
        }
